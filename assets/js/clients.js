...
function displayCustomer(e) {
  ...
  xhr.open(
    "GET",
    "controllers/ajax/ajaxListCustomerController.php?displayid=" + idCustomer,
    true
  );
  xhr.send();
}
...
function listCustomerPattern(d) {
  let pattern =
    ...
    '<button type="button" name="info" class="infoCustomer" id="infoCustomer" title="fiche d\'information" data-id="' +
    d.id +
    '" onclick="displayCustomer(this)"><i class="fas fa-eye"></i></button>' +
    "</td>" +
    '<td class="border">' +
    '<a class="editCustomerButton" href="./Modifier-Employer-' +
    d.id +
    '"><button title="modifier la fiche"><i class="fas fa-edit"></i></button></a>' +
    "</td>" +
    '<td class="border">' +
    '<button type="button" id="hoverDanger" class="deleteCustomer" title="suppression de la fiche" data-id="' +
    d.id +
    '" onclick="deleteCustomerById(this)"><i class="fas fa-trash-alt"></i></button>' +
    "</td>" +
    "</tr>";
  return pattern;
}
...
confirmDelete.addEventListener("click", () => {
  ...
  xhr.open(
    "GET",
    "controllers/ajax/ajaxListCustomerController.php?deleteid=" + idCustomer,
    true
  );
  xhr.send();
});
